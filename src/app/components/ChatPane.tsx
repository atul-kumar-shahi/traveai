...
export default function ChatPane({ messages, inputValue, onInputChange, onSendMessage }: ChatPaneProps) {
    ...
    if (text.includes('\n')) {
      ...
      return parts.map((part, index) => {
        if (part.trim().startsWith('•')) {
          ...
          return (
            <li key={index} className="ml-5">{part.replace('•', '').trim()}</li>
          );
        } else if (part.endsWith(':')) {
          // This is a heading
          return (
            <p key={index} className="mt-2 font-semibold">{part}</p>
          );
        } else {
          // Regular paragraph
